...
class GeminiLLM():
    ...
    def query_with_image(self, image, prompt=None):
        ...
        response = self.chat.send_message(prompt)
        self.chat.history = self.chat.history[:-1]
        temp_history = self.chat.history[-1]
        self.chat.history = self.chat.history[:-1]
        
        img = PIL.Image.open(image)

        described = self.vision_model.generate_content(["Describe what's on the image", img], stream=True)
        described.resolve()
        ...
        self.chat.history.append(temp_history)
        self.chat.history.append(response.candidates[0].content)
        self.pop_history()
